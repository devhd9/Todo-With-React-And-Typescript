...
function OverlayInputForm({
  addItemInTodoListHandler,
  isNewItem,
  currentItemData,
}: OverlayInputFormPropType) {
  const [inputValue, setInputValue] = useState<string>(
    currentItemData ? currentItemData.title : "",
  );
  function saveValue() {
    const dataObj = {
      isNewItem,
    ...
    <div className={`${styles.overlay} pointer-hover"`} id="overlay">
      <div className={`${styles["form-container"]} ${styles["form-width"]}`}>
        ...
        <div className={styles["input-container"]}>
          ...
          />
          <label htmlFor="title" className={styles["input-label"]}>
            Title*
          </label>
        </div>
        <div style={{ textAlign: "center", marginTop: "10px" }}>
          <span
            onClick={saveValue}
            className={`${styles["tick-icon"]} pointer-hover`}
          >
            <TickIcon size="30" />
          </span>
        </div>
